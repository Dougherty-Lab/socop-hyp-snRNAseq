...
write.csv(t, "../ej_results/GEX/250301_tableofcellspergenotype.csv",
          quote = FALSE,
          row.names = TRUE)
```
...
dds <- DESeqDataSetFromMatrix(round(cluster_counts),
  ...
  write.csv(sig_res, paste0("../ej_results/GEX/Geno/", name.cleaned, " signif genes.csv"),
            quote = FALSE,
            row.names = TRUE)
}
```
...
for (name in cluster_names){
  ...
  name.cleaned <- gsub('/', '_', name)
  write.csv(sig_res, paste0("../ej_results/GEX/Sex/", name.cleaned, " signif genes.csv"),
            quote = FALSE,
            row.names = TRUE)
}
```
...
for (name in cluster_names){
  ...
  name.cleaned <- gsub('/', '_', name)
  write.csv(sig_res, paste0("../ej_results/GEX/Learner/", name.cleaned, " signif genes.csv"),
            quote = FALSE,
            row.names = TRUE)
}
```
