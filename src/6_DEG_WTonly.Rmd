...
WT.only <- subset(df, subset = genotype == "WT")
...
```{r name_cleaning}
WT.only@meta.data$sample <- as.factor(gsub("_", "-", WT.only@meta.data$sample))
WT.only@meta.data$cell_type <- Idents(WT.only)
WT.only@meta.data$cell_type <- as.factor(gsub("/", "-", WT.only@meta.data$cell_type))
WT.only@meta.data$cell_type <- as.factor(gsub(" ", "-", WT.only@meta.data$cell_type))
WT.only@meta.data$cell_type <- as.factor(gsub("_", "-", WT.only@meta.data$cell_type))
```


```{r geno table}
genotypesTable <- table(WT.only@meta.data$genotype)
genotypesTable
```

```{r sex table}
sexesTable <- table(WT.only@meta.data$sex)
sexesTable
```

```{r make sce WT.only}
counts <- WT.only@assays$RNA@counts
metadata <- WT.only@meta.data
...
write.csv(t, "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/250301_tableofcellspergenotype.csv",
          quote = FALSE,
          row.names = TRUE)
```
...
for (name in cluster_names) {
  ...
  if (length(unique(cluster_metadata$sex)) > 1) {
    ...
    write.csv(sig_res, paste0("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Sex/", name.cleaned, " signif genes.csv"),
              quote = FALSE,
              row.names = TRUE)
...
for (name in cluster_names){
  ...
  if (length(unique(cluster_metadata$Sac_Learner)) > 1) {
    ...
    # Write results to file
    name.cleaned <- gsub('/', '_', name)
    write.csv(sig_res, paste0("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Learner/", name.cleaned, " signif genes.csv"),
              quote = FALSE,
              row.names = TRUE)
...
### Bar plot
```{r DEG results WT.only for sex}
file_list <- list.files("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Sex/", pattern = "signif genes.csv", full.names = TRUE)

results_WT.only <- data.frame(file = character(), 
                         F_enrich = integer(), 
                         M_enrich = integer(), 
                         stringsAsFactors = FALSE)

for (file in file_list) {
  file_WT.only <- read.csv(file)
  F_enrich <- sum(file_WT.only$log2FoldChange < 0)
  M_enrich <- sum(file_WT.only$log2FoldChange > 0)
  
  results_WT.only <- rbind(results_WT.only, 
...
results_WT.only$file <- sub("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Sex//", "", results_WT.only$file)
results_WT.only$file <- sub(" signif genes.csv", "", results_WT.only$file)

write.csv(results_WT.only, file = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/DEG table.csv", sep = ",")
```

```{r plot DEG results sex}
results_WT.only$cluster <- results_WT.only$file
results_WT.only$cluster <- factor(results_WT.only$cluster, levels = rev(unique(results_WT.only$cluster)))

# First, make F_enrich negative to flip it to the left side
results_WT.only$F_enrich_neg <- -results_WT.only$F_enrich

# Create the combined plot
comb.plot <- ggplot(results_WT.only) +
  ...
  theme(        axis.text.y = element_text(color = rep(c("#000000", "#555555"), 
                                             length.out = length(unique(results_WT.only$cluster))))) +
  geom_vline(xintercept = 0, linewidth = 0.2)

comb.plot
ggsave('/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/intermed-figs/250301_DEGsbySexall.png',
       device = png,
       width = 6,
       height = 6)
```
...
sig_results_WT.only <- results_WT.only %>%
  dplyr::filter(cluster %in% sig_cluster_names) %>%
  arrange(match(cluster, sig_cluster_names))


sig_results_WT.only$cluster <- factor(sig_results_WT.only$cluster, levels = rev(sig_cluster_names))


sig_plot <- 
# Create the combined plot
ggplot(sig_results_WT.only) +
  # Male enrichment (right side)
  geom_bar(aes(x = M_enrich, y = cluster, fill = "M"),
           stat = "identity",
           linewidth = 0.2) +
...
sig_plot
 ...
 ggsave(filename = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/intermed-figs/250301_DEGsbysex_sig.png",
        width = 8,
        height = 12,
        dpi = 300)
```
...
file_list <- list.files("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Sex", pattern = "signif genes.csv", full.names = TRUE)
file_list <- sub("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Sex/", "", results_WT.only$file)


concatenated_data <- data.frame()

for (file in file_list){
  current_data <- read.csv(paste0("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Sex/", file, " signif genes.csv"), sep = ',', header = TRUE)
...
write.csv(concatenated_data, file = '/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Sex/250301_DEGs_table.csv')
```
...
write_csv(uniqueDEGs, file = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Sex/250301_uniqueDEGs.csv")
```
...
ggsave(filename = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/250301_PO-MEA-BST-Mixed-Gaba_volcano.pdf", 
       width = 6,
       height = 4,
       device = pdf)
```
...
ggsave(filename = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/250301_PO-MEA-BST-Mixed-Gaba_volcano_noXYgenes.pdf", 
       width = 6,
       height = 4,
       device = pdf)
```
...
### Bar plot
```{r DEG results WT.only for sac learner}
file_list <- list.files("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Learner", pattern = "signif genes.csv", full.names = TRUE)
...
for (file in file_list) {
  file_WT.only <- read.csv(file)
  L_enrich <- sum(file_WT.only$log2FoldChange < 0)
  NL_enrich <- sum(file_WT.only$log2FoldChange > 0)
...
results_WT.only$file <- sub("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Learner/", "", results_WT.only$file)
results_WT.only$file <- sub(" signif genes.csv", "", results_WT.only$file)

write.csv(results_WT.only, file = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Learner/DEG table.csv", sep = ",")
```

```{r plot DEG results sac learner}
rownames(results_WT.only) <- results_WT.only$file
results_WT.only$cluster <- factor(results_WT.only$file)
results_WT.only <- drop_na(results_WT.only)

# First, make NL_enrich negative to flip it to the left side
results_WT.only$NL_enrich_neg <- -results_WT.only$NL_enrich
...
comb.plot
ggsave('/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/intermed-figs/250301_DEGsbyLearnerall.png',
       device = png,
       width = 6,
       height = 6)
```
...
sig_results_WT.only <- results_WT.only[((results_WT.only$NL_enrich != 0) + (results_WT.only$L_enrich != 0)) >1, ]

# Extract the BaselineProp.clusters for the significant results
significant_cluster_names <-sort(rev(as.character(sig_results_WT.only$file)))

# Print the list of significant clusters
print(significant_cluster_names)


sig_results_WT.only$cluster <- factor(sig_results_WT.only$cluster, 
                                levels = rev(significant_cluster_names))
```

```{r plot sig DEG results for sac learner}
# Create the combined plot
sig.comb.plot <- ggplot(sig_results_WT.only) +
  # Male enrichment (right side)
  geom_bar(aes(x = L_enrich, y = cluster, fill = "Learner"),
           stat = "identity",
           linewidth = 0.2) +
...
sig.comb.plot
 ...
 ggsave(filename = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/250301_DEGsbylearner.png",
        width = 8,
        height = 12,
        dpi = 300)
```
...
file_list <- sub("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Learner/", "", results_WT.only$file)

concatenated_data <- data.frame()

for (file in file_list){
  current_data <- read.csv(paste0("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Learner/", file, " signif genes.csv"), sep = ',', header = TRUE)
  if(dim(current_data)[1] > 0) {
  current_data$cellType <- sub("signif genes.csv", "", file)
  concatenated_data <- rbind(concatenated_data, current_data)
  }
...
write.csv(concatenated_data, file = '/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Learner/250301-DEGs_table.csv')
```
...
write_csv(uniqueDEGs, file = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Learner/250301_uniqueDEGs.csv")
```
...
ggsave(filename = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/250301_PH-LHA-PVH-SO-PVa-Nxph4-Glut_learner_volcano.png",
       width = 6,
       height = 4,
       device = png)
```
...
# GO Analysis
go_table <- read.csv(paste0("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Learner/", name, " signif genes.csv"), sep = ',',
                     header = TRUE)
...
ggsave(paste0("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Learner/", name, " GO enrichment dotplot.png"),
       device = png,
       width = 5,
       height = 7)
```
...
p
# Save plot
ggsave("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/GEX_WTonly/Learner/cluster_GO_enrichment_dotplot.png",
       plot = p,
       width = 12,
       height = 10)
```
