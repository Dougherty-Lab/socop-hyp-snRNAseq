...
nxph4.subset <- subset(combined.neuronal,
...
```{r re-process subset}
nxph4.subset <- NormalizeData(nxph4.subset,
                                    normalization.method = "LogNormalize",
                                    scale.factor = 10000)

nxph4.subset <- FindVariableFeatures(nxph4.subset,
                                           selection.method = "vst",
                                           nfeatures = 3000)

all.genes <- rownames(nxph4.subset)
nxph4.subset <- ScaleData(nxph4.subset, features = all.genes)

nxph4.subset <- RunPCA(nxph4.subset,
                             npcs = 50,
                             features = VariableFeatures(object = nxph4.subset))

nxph4.subset <- FindNeighbors(nxph4.subset, dims = 1:50)

nxph4.subset <- FindClusters(nxph4.subset, 
                             resolution =  c(0.01, 0.05, 0.1, 0.2, 0.4, 0.5, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2))
```

```{r clustree for subset, fig.height=8}
clustree(nxph4.subset, prefix = 'RNA_snn_res.', layout = "sugiyama")
```
Resolution 0.1 appears to be appropriate and not over-cluste#ED6677.

```{r run umap}
Idents(nxph4.subset) <- nxph4.subset$RNA_snn_res.0.1

nxph4.subset <- RunUMAP(nxph4.subset, dims = 1:50, n.components = 2L)
```

```{r plot umap, fig.height=3, fig.width=3}
DimPlot_scCustom(nxph4.subset,
...
ggsave("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/Nxph4subset/250304_Nxph4subset_umap.png",
      device = png,
      width = 3,
      height = 3,
      dpi = 300)
...
ggsave("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/Nxph4subset/250304_Nxph4subset_umap_withlabel.pdf",
      device = pdf,
      width =5,
      height = 10,
      dpi = 300)
...
```{r find markers}
nxph4.subset.markers.all <- FindAllMarkers(nxph4.subset,
                                             only.pos = FALSE,
                                             min.pct = 0.25,
                                             logfc.threshold = 0.5,
                                             return.thresh = 0.01)

nxph4.subset.markers.all$`pct.1-pct.2` <- nxph4.subset.markers.all$pct.1 - nxph4.subset.markers.all$pct.2

dupes_counts <- table(nxph4.subset.markers.all$gene)

nxph4.subset.markers.all$dups <- dupes_counts[match(nxph4.subset.markers.all$gene, names(dupes_counts))]

saveRDS(nxph4.subset.markers.all,file = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/Nxph4subset/250304_markers_nxph4subcluster_res0.1.rds")

write.csv(nxph4.subset.markers.all,file = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/Nxph4subset/250304_markers_nxph4subcluster_res0.1.csv")
```



```{r use existing ABC lbels to make table}
clusters <- nxph4.subset@meta.data$RNA_snn_res.0.1
ABC_class_names <- nxph4.subset@meta.data$ABC_class_name
ABC_subclass_names <- nxph4.subset@meta.data$ABC_subclass_name
ABC_supertype_name <-nxph4.subset@meta.data$ABC_supertype_name
ABC_cluster_name <- nxph4.subset@meta.data$ABC_cluster_name
...
clusters <- nxph4.subset@meta.data$RNA_snn_res.0.1
predicted <- nxph4.subset@meta.data$hypomap_predicted
...
nxph4.subset <- BuildClusterTree(nxph4.subset, dims = 1:50, reorder = TRUE)

PlotClusterTree(nxph4.subset)
```
...
nxph4.subset <- RenameIdents(nxph4.subset, cell_types)
nxph4.subset@meta.data$nxph4_cell_type <- Idents(nxph4.subset)
```

# Props 

```{r add nxph4 subsets back to combined.neuronal}
nxph4.subset@meta.data$subsetted_cell_type <- Idents(nxph4.subset)

new_cell_types <- Idents(nxph4.subset)
names(new_cell_types) <- colnames(nxph4.subset)

combined.neuronal@meta.data$subsetted_cell_type <- Idents(combined.neuronal)

combined.neuronal$subsetted_cell_type <- as.character(combined.neuronal$subsetted_cell_type)
combined.neuronal$subsetted_cell_type[names(new_cell_types)] <- as.character(new_cell_types)

Idents(combined.neuronal) <- combined.neuronal$subsetted_cell_type
levels(Idents(combined.neuronal))
```

```{r get props df}
# Extract the cell type proportions and transform them
props <- getTransformedProps(cluster = combined.neuronal$subsetted_cell_type, 
...
sample_info <- data.frame(sample = combined.neuronal$sample, 
                          ...
                          clusters = combined.neuronal$subsetted_cell_type,
...
write.csv(subset_df, file = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/Nxph4subset/250304_rewardsbyprops_withNxph4subset.csv")
```
...
write.csv(subset_df_bysex, file = "/scratch/jdlab/simona/socop-hyp-snRNAseq/results/Nxph4subset/250304_rewardsbyprops_Nxph4subset_sex.csv")
```
...
```{r, fig.height=8}
# Open PDF device
pdf("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/Nxph4subset/250304_PVNwithinNxph4_propsbyrewards.pdf", width = 6, height = 8)
...
# Calculate R-squa#ED6677 values for each fit
r2_all <- summary(fit_all)$r.squa#ED6677
r2_male <- summary(fit_male)$r.squa#ED6677
r2_female <- summary(fit_female)$r.squa#ED6677
...
```{r, fig.height=8}
# Open PDF device
pdf("/scratch/jdlab/simona/socop-hyp-snRNAseq/results/Nxph4subset/250304_LHAwithinNxph4_propsbyrewards.pdf", width = 6, height = 8)
...
```{r}
saveRDS(nxph4.subset,file = "/scratch/jdlab/simona/socop-hyp-snRNAseq/data/250304_Nxph4_subset_res0.1.rds")
```
